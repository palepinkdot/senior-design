...
export class UserResolver {
	...
	async updateUserInfo(@Arg("options") options: string, @Ctx() { req }: MyContext): Promise<UserResponse> {
	...
	@Mutation(() => UserResponse)
	async changeUserPassword(@Arg("token") token: string, @Arg("newPassword") newPassword: string, @Ctx() { redis, req }: MyContext): Promise<UserResponse> {
	...
	@Mutation(() => Boolean)
	async forgotUserPassword(@Arg("email") email: string, @Ctx() { redis }: MyContext) {
		...
		await sendEmail(email, `<a href="${corsOrigin}/change-password/${token}">reset password</a>`);
	...
	@Mutation(() => UserResponse)
	async loginUser(@Arg("usernameOrEmail") usernameOrEmail: string, @Arg("password") password: string, @Ctx() { req }: MyContext): Promise<UserResponse> {
